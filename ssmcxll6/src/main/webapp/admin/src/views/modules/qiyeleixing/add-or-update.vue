<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="企业类型" prop="qiyeleixing">
          <el-input v-model="ruleForm.qiyeleixing" 
              placeholder="企业类型" clearable  :readonly="ro.qiyeleixing"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="企业类型" prop="qiyeleixing">
              <el-input v-model="ruleForm.qiyeleixing" 
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `qiyeleixing/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.qiyeleixingCrossAddOrUpdateFlag = false;
                  this.parent.search();
